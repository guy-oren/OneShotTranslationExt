...
class Solver(object):
    ...
    def merge_images(self, sources, targets, k=10):
        ...
        row = int(np.sqrt(self.svhn_batch_size)) + 1
    ...
    def train(self):
        ...
        for step in range(self.train_iters + 1):
            ...
            if self.svhn_batch_size > self.config.max_items:
                exit(-1)
            elif self.svhn_batch_size == self.config.max_items:
                svhn = fixed_svhn
            elif self.svhn_batch_size < self.config.max_items:
                counter += 1
                if counter * self.svhn_batch_size >= self.config.max_items:
                    svhn_iter = iter(self.svhn_loader)
                    counter = 0
            ...
            self.reset_grad()
            es = self.g11.encode(svhn, svhn=True)
            fake_svhn = self.g11.decode(es, svhn=True)
            g_loss += torch.mean((svhn - fake_svhn) ** 2)

            if self.config.one_way_cycle:
                es = self.g11.encode(svhn, svhn=True)
                fake_mnist = self.g11.decode(es)
                es = self.g11.encode(fake_mnist)
                fake_svhn = self.g11.decode(es, svhn=True)
                g_loss += torch.mean((svhn - fake_svhn) ** 2)
            ...
            if (step + 1) % self.sample_step == 0:
                es = self.g11.encode(fixed_svhn, svhn=True)
                fake_mnist_var = self.g11.decode(es)
