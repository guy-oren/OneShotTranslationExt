...
class G11(nn.Module):
    def __init__(self, conv_dim=64):
        ...
        self.conv1_mnist_m = conv(3, conv_dim, 4)
        ...
        self.deconv2_mnist_m = deconv(conv_dim, 3, 4, bn=False)

    def forward(self, x, mnist_m=False):
        if mnist_m:
            out = F.leaky_relu(self.conv1_mnist_m(x), 0.05)  # (?, 64, 16, 16)
        else:
        ...
        if mnist_m:
            out = F.tanh(self.deconv2_mnist_m(out))  # (?, 3, 32, 32)
        else:
            out = F.tanh(self.deconv2(out))  # (?, 3, 32, 32)

        return out

    def encode(self, x, mnist_m=False):
    ...
    def decode(self, out, mnist_m=False):
    ...
    def encode_params(self):
        layers_basic = list(self.conv1_mnist_m.parameters()) + \
    ...
    def decode_params(self):
        layers_basic = list(self.deconv2_mnist_m.parameters()) + \
    ...
    def unshared_parameters(self):
        return list(self.deconv2_mnist_m.parameters()) + list(self.conv1_mnist_m.parameters()) + \
